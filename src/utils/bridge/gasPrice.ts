  ...
  try {
    ...
    for (const speedType in json) {
      if (Object.prototype.hasOwnProperty.call(json, speedType)) {
        returnJson[speedType] = utils.parseUnits(json[speedType].toFixed(2), 'gwei')
      }
    }
...
class GasPriceStore {
  ...
  constructor() {
    ...
    this.medianHistoricalPrice = utils.parseUnits(REACT_APP_GAS_PRICE_FALLBACK_GWEI || '0', 'gwei')

    this.fastGasPrice = utils.parseUnits(REACT_APP_GAS_PRICE_FALLBACK_GWEI || '0', 'gwei')
    this.speedType = REACT_APP_GAS_PRICE_SPEED_TYPE || DEFAULT_GAS_PRICE_SPEED_TYPE
    this.updateInterval = Number(REACT_APP_GAS_PRICE_UPDATE_INTERVAL || DEFAULT_GAS_PRICE_UPDATE_INTERVAL)
    this.updateGasPrice()
  ...
  async updateHistoricalPrice() {
    const response = await axios.get(`https://ethgas.watch/api/gas/trend?hours=168`)
    if (response.status !== 200) {
      throw new Error(`Fetch gasPrice from ethgasAPI failed!`)
    }
    const { normal } = response.data
    this.medianHistoricalPrice = utils.parseUnits(median(normal).toString(), 'gwei')
    this.lowestHistoricalPrice = utils.parseUnits(lowest(normal).toString(), 'gwei')
